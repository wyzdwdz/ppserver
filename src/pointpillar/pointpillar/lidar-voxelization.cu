...
static __global__ void generateVoxels_random_kernel(
  const float* points,
  size_t points_size,
  float min_x_range,
  float max_x_range,
...
static __global__ void generateBaseFeatures_kernel(
  unsigned int* mask,
  float* voxels,
  int grid_y_size,
  int grid_x_size,
...
)
{
  dim3 threads = {32, 32};
  dim3 blocks = {
    (grid_x_size + threads.x - 1) / threads.x,
...
static __global__ void generateFeatures_kernel(
  float* voxel_features,
  unsigned int* voxel_num,
  unsigned int* voxel_idxs,
  unsigned int* params,
...
)
{
  ...
  if (point_idx < pointsNumSM[pillar_idx_inBlock])
  {
    pillarOutSM[pillar_idx_inBlock][point_idx][0] =
      __float2half(pillarSM[pillar_idx_inBlock][point_idx].x);
    pillarOutSM[pillar_idx_inBlock][point_idx][1] =
      __float2half(pillarSM[pillar_idx_inBlock][point_idx].y);
    pillarOutSM[pillar_idx_inBlock][point_idx][2] =
      __float2half(pillarSM[pillar_idx_inBlock][point_idx].z);
    pillarOutSM[pillar_idx_inBlock][point_idx][3] =
      __float2half(pillarSM[pillar_idx_inBlock][point_idx].w);
...
nvtype::Int3 VoxelizationParameter::compute_grid_size(
...
public:
  ...
  bool init(VoxelizationParameter param)
  {
    ...
    mask_size_ =
      param_.grid_size.z * param_.grid_size.y * param_.grid_size.x * sizeof(unsigned int);
    ...
    features_input_size_ =
      param_.max_voxels * param_.max_points_per_voxel * 10 * sizeof(nvtype::half);
  ...
  virtual void forward(const float* _points, int num_points, void* stream) override
  {
    ...
    checkRuntime(generateVoxels_random_launch(
      _points,
      num_points,
      param_.min_range.x,
      param_.max_range.x,
    ...
    checkRuntime(generateBaseFeatures_launch(
      mask_,
      voxels_,
      param_.grid_size.y,
      param_.grid_size.x,
    ...
    checkRuntime(generateFeatures_launch(
      voxel_features_,
      voxel_num_,
      voxel_idxs_,
      params_input_,
  ...
  virtual const nvtype::half* features() override
  {
    return features_input_;
  }

  virtual const unsigned int* coords() override
  {
    return voxel_idxs_;
  }

  virtual const unsigned int* params() override
  {
    return params_input_;
  }
