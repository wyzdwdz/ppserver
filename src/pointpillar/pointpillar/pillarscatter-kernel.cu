...
)
{
  ...
  {
    pillarSM[i][threadIdx.x] = pillar_features_data
      [(blockIdx.x * PILLARS_PER_BLOCK + i) * PILLAR_FEATURE_SIZE + threadIdx.x];
  }
  __syncthreads();
...
__global__ void pillarScatterFloatkernel(
...
)
{
  ...
  pillarScatterHalfkernel<<<blocks, threads, 0, stream>>>(
...
)
{
  ...
  pillarScatterFloatkernel<<<blocks, threads, 0, stream>>>(
    pillar_features_data,
    coords_data,
    params_data,
    featureX,
